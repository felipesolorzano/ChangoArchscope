...
function moduleFromQuery(value: unknown): string | null {
  return typeof value === "string" && value.length > 0 ? value : null;
}

export function architectureWebRoutes(): Router {
  const router = Router();
  ...
  router.get("/graph.json", (request, response, next) => {
    try {
      response.status(200).json(
        buildArchitectureGraph(getArchitectureConfig(), reader, {
  ...
  router.get("/check.json", (request, response, next) => {
    try {
      response.status(200).json(
        checkArchitecture(getArchitectureConfig(), reader, {
          target: targetFromQuery(request.query.target),
          module: moduleFromQuery(request.query.module),
