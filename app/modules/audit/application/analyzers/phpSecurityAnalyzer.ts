import type { PhpFileStructure, PhpSecurityIssue } from "../../domain/value-objects/PhpFileStructure.js";
import type { AuditFinding, AuditFindingSeverity } from "../../domain/value-objects/AuditSnapshot.js";

const SEVERITY_BY_RULE: Record<PhpSecurityIssue["rule"], AuditFindingSeverity> = {
  "eval-usage": "critical",
...
const MESSAGE_BY_RULE: Record<PhpSecurityIssue["rule"], string> = {
...
export function phpSecurityAnalyzer(files: PhpFileStructure[]): AuditFinding[] {
  return files.flatMap((file) => file.securityIssues.map((issue) => buildFinding(file.file, issue)));
}

function buildFinding(file: string, issue: PhpSecurityIssue): AuditFinding {
  return {
    category: "security",
    rule: issue.rule,
