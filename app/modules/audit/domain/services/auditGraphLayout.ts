import type {
  AuditGraphAccent,
  AuditGraphPosition,
  AuditGraphSeverityMix,
  AuditGraphTone,
} from "../value-objects/AuditGraph.js";
...
const KNOWN_ACCENTS: AuditGraphAccent[] = [
  "security",
  "database",
  "complexity",
  "testing",
...
export function foldSeverityMix(bySeverity: Record<string, number>): AuditGraphSeverityMix {
  return {
...
export function dominantAccent(byCategory: Record<string, number>): AuditGraphAccent {
  ...
  return KNOWN_ACCENTS.includes(winner as AuditGraphAccent) ? (winner as AuditGraphAccent) : "mixed";
}

export function toneForSeverity(bySeverity: Record<string, number>): AuditGraphTone {
...
export function sizeForRisk(risk: number, maxRisk: number): number {
  if (maxRisk <= 0) {
...
export function overviewPositions(count: number): AuditGraphPosition[] {
...
export function gridPositions(count: number, columns: number): AuditGraphPosition[] {
