...
import { listDirectories, nowIso, relativePosix, walkFiles } from "./fsUtils.mjs";
import { phpImports } from "./phpImports.mjs";
import { checkResponse, issue, report } from "./reports.mjs";

export function buildLaravelGraph(config, onlyModule = null) {
  const modulesPath = config.laravel.modulesPath;
  const modules = laravelModules(config, onlyModule);
  const nodes = [];
  ...
  for (const [module, modulePath] of modules) {
    ...
    for (const file of walkFiles(modulePath, [".php"])) {
      ...
      phpImports(file).forEach((importItem, importIndex) => {
        const target = targetForLaravelImport(config, importItem.import);
        ...
        if (target.file) {
          ...
          const targetLayer = existsSync(targetAbsoluteFile) ? laravelLayerFor(config, targetModulePath, targetAbsoluteFile) : null;
        ...
        edges.push({
          ...
          target: targetNodeId,
          type: "import",
          label: importItem.import.split("\\").pop(),
...
export function checkLaravelArchitecture(config, onlyModule = null, failOnCoupling = true) {
  const reports = laravelModules(config, onlyModule).map(([module, modulePath]) => {
    const scannedFiles = walkFiles(modulePath, [".php"]);
    const violations = [];
    const couplings = [];
    ...
    for (const file of scannedFiles) {
      ...
      for (const importItem of phpImports(file)) {
        const coupling = laravelCouplingFor(config, module, layer, file, importItem);
...
function laravelModules(config, onlyModule) {
  return listDirectories(config.laravel.modulesPath)
    .map((modulePath) => [path.basename(modulePath), modulePath])
    .filter(([module]) => !onlyModule || studly(onlyModule) === module);
}

function laravelLayerFor(config, modulePath, file) {
...
function targetForLaravelImport(config, importName) {
  ...
  return {
    module,
    file: existsSync(absoluteFile) ? relativeFile : null,
  };
}

function laravelCouplingFor(config, module, layer, file, importItem) {
...
function laravelRoleFor(filePath, layer) {
...
function moduleNode(id, module, modulePath) {
  return {
    id,
    type: "module",
    label: module,
